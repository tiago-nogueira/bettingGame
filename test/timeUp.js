...
contract("Betting-timeUp", accounts => {
	let Instance;
	let bet = config.bet;
	...
	it("reveals first number", () => {
		return Instance.revealNum(123, { from: accounts[1] });
	});
	...
	it("calls the 'timeUp'", () => {
		...
		.then(receipt => {
			...
			return checkReceipt(receipt, "Winner", accounts[1], 2*bet);
		});
	});
});